...
def debug(name, img):
    if not os.path.exists(f"./debug/{name}.png"):
...
class Normalize(object):
    ...
    def __call__(self, rendering_images):
        ...
        # Debug            
        # debug("normalize", rendering_images[0]*255)
...
class RandomPermuteRGB(object):
    def __call__(self, rendering_images):
        ...
        # Debug            
        # debug("random_rgb", img*255)

        return rendering_images


class RandomErasing(object):
    def __init__(self, img_size, min_erasing, max_erasing, p_erasing):
        self.img_h = img_size[0]
        self.img_w = img_size[1]
        self.min_erasing = int(min_erasing)
        self.max_erasing = int(max_erasing)
        self.p_erasing = float(p_erasing)

    def __call__(self, rendering_images):

        p = random.uniform(0, 1)*100

        if(p>self.p_erasing) or len(rendering_images) == 0:
            return rendering_images
        
        img_h, img_w, img_c = rendering_images[0].shape
        processed_images = np.empty(shape=(0, self.img_h, self.img_w, img_c))

        for img_idx, img in enumerate(rendering_images):
            
            # Threshold
            img_bin = cv2.normalize(img, None, 255, 0, cv2.NORM_MINMAX, cv2.CV_8U)
            img_bin = cv2.cvtColor(img_bin, cv2.COLOR_BGR2GRAY)
            ret,thresh = cv2.threshold(img_bin,0,255,cv2.THRESH_BINARY)
            
            # Max contour
            contours,hierarchy = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
            cnt = max(contours, key = cv2.contourArea)
            b_left, b_top, b_w, b_h = cv2.boundingRect(cnt)

            erasing_percentage = np.random.randint(self.min_erasing, self.max_erasing)
            mask_area = (b_h*b_w)*erasing_percentage/100
            ...
            if(b_top < b_bot-mask_h):
                mask_top = np.random.randint(b_top, b_bot-mask_h)
            if(b_left < b_right-mask_w):
                mask_left = np.random.randint(b_left, b_right-mask_w)


            # Generate occluded
            mask = np.zeros((img_h, img_w, 1), dtype="uint8")
            mask[mask_top:mask_top+mask_h, mask_left:mask_left+mask_w, :] = 1

            occ_image = img*(1. - mask)

            processed_images = np.append(processed_images, [occ_image], axis=0)
...
class CenterCrop(object):
    ...
    def __call__(self, rendering_images, bounding_box=None):
        ...
        for img_idx, img in enumerate(rendering_images):
            ...
            # Debug
            # debug("center_crop", processed_image*255)
...
class RandomCrop(object):
    ...
    def __call__(self, rendering_images, bounding_box=None):
        ...
        for img_idx, img in enumerate(rendering_images):
            ...
            # Debug
            # debug("random_crop", processed_image*255)
...
class ColorJitter(object):
    ...
    def __call__(self, rendering_images):
        ...
        for img_idx, img in enumerate(rendering_images):
            ...
            # Debug
            # debug("color_jitter", processed_image*255)
...
class RandomNoise(object):
    ...
    def __call__(self, rendering_images):
        ...
        for img_idx, img in enumerate(rendering_images):
            ...
            # Debug
            # debug("random_noise", processed_image*255)
